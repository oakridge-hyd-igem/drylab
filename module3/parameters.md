...
| `K` | Half-max (activation) analyte concentration | µM | 1 | 0.1 – 10 | Sensor-dependent (Cu/nitrate promoter not finalised); ESTIMATED, order-of-magnitude for a metal-responsive promoter |
| `β_leak` | Basal integrase production at A = 0 (leak) | copies·cell⁻¹·h⁻¹ | **10 (= 1%·β_max)** | 1 – 100 (10⁻³ – 10⁻¹·β_max) | Hsiao et al. 2016 fit leak = 1–2% of production. (Alon Ch. 2 textbook default ~10⁻³·β_max is the low end of the range.) **leak-suppression target** |
...
| `N` | Population size (agents simulated) | cells | 1000 | 100 – 100 000 | Simulation parameter; N sets the finest resolvable F (≥ 1/N) |
...
- Steady-state leak integrase: `I_leak = β_leak/γ` ≈ 33 (untagged) / 7.5 (LVA) copies —
  the false-positive pressure. Note LVA cuts steady-state leak integrase ~4×.
...
5. **`K` and `n` — sensor promoter parameters.** The analyte is not finalised
   (copper vs nitrate, originally arsenic), so `K` is a placeholder order-of-magnitude and
   `n` a generic cooperative default. These shift the dose axis but, because `A_pulse` is
